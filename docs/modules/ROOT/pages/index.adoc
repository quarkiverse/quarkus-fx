...
[source]
----
%GRAALVM_HOME%\bin\java -agentlib:native-image-agent=config-output-dir=tracing-output -jar quarkus-run.jar
----
...
Copy `tracing-output/reachability-metadata.json` to `src/main/resources/META-INF/native-image/{groupId}/{artifcatId}/reachability-metadata.json`